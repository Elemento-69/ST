...
function setTable() {
    $.ajax({
        url: `${urlbase}/api/ActaInicio/ObtenerActaInicio/2022/B-014`,
        success: (val) => {
...
$("#btnIngresarActaSuper").on("click", () => {
    let anio = $('#AnioproyectoSuper').val();
    let Proyecto = $("#proyectoSuper").val();
    let id = $("#ID").val();
    let codigo = 'SUP'+ id +'-' + Proyecto + '-' + anio;
    let data = {
        "CodigoActa": codigo,
    ...
    $.ajax({
        url: `${urlbase}/api/ActaInicio/AgregarActaInicioSupervisora`,
        headers: {
            "Authorization": "Bearer " + token,
            "Content-Type": "application/json",
        },
