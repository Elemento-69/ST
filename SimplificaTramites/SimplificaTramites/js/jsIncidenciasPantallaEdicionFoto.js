...
function initMap() {
    ...
    infoWindow = new google.maps.InfoWindow({
        content: "Seleccione en el mapa la ubicaci&oacuten donde se tom&oacute la foto",
...
function fnMostrarRegistro() {
    const url = baseHostURL + "api/IncidenciasApp/obtenerDatosDeFoto/" + vIdIncidencia + "/" + vIdArchivoMultimedia;
        ...
        .then(data => {
            ...
            if (datos.length > 0) {
                datos.forEach(function (registro) {
                    ...
                    document.getElementById("image-foto").src =thumbnail + "Tipo=8&MaxPixels=0&Fotografia=" + vNombreArchivo + "&ID=" + vIdIncidencia + "&random=" + new Date().getTime();
                    //document.getElementById("image-foto").src = registro.urlArchivoMultimedia + "&random=" + new Date().getTime();
                    $.LoadingOverlay("hide");
                })
            }
        })
...
function fnRotarFoto(rotarIzquierda) {
    ...
    $.ajax({
        type: "POST",
        url: "frmIncidenciasEdicionFoto.aspx/fRotarFoto",
        data: dataJSONt,
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
            var nombreArchivoNuevo = data.d;
            var rutaFoto = thumbnail + "Tipo=8&MaxPixels=0&Fotografia=" + vNombreArchivo + "&ID=" + vIdIncidencia + "&random=" + new Date().getTime();
            ...
            //$.ajax({
            //    type: "POST",
            //    url: "frmIncidenciasEdicionFoto.aspx/fReemplazarArchivoFoto",
            //    data: dataJSONRotar,
            //    contentType: "application/json; charset=utf-8",
            //    dataType: "json",
            //    success: function (data) {
