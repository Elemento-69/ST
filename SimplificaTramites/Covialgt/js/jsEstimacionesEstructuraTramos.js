...
function fnConsultarToken() {
    ...
    $.ajax({
        type: "POST",
        url: "../Estimaciones/frmEstimacionesEstructuraTramos.aspx/fObtenerToken",
        data: '',
    ...
    fetch(proxyurl + url, {
        ...
        .then(val => {
            ...
            $(".estimacion_detail-btn").on("click", ({ currentTarget }) => {
                ...
                var urlDetalle = baseHostURL + "api/Dashboard/MontoEjecutadoTramos/" + $("#cmbPlanAnual1").val() + "/" + $("#cmbProyecto1").val() + "/" + currentTarget.dataset.id;
                //alert(urlDetalle);
                $.ajax({
                    url: urlDetalle,
                    headers: {
