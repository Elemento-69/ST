$(document).ready(function () {
    ...
    $("#btnGenerar").click(function () {
        ...
        let desde = moment($('#desdeInput').val(), "DD-MM-YYYY").isValid() ? $("#desde-dp").datetimepicker("date").format("YYYY-MM-DD").split('-') : ''
        if (desde.length == 0 || anio.length == 0) {
            Swal.fire("Atención", "Deben completarse los datos requeridos para la generación de reporte", "error")
            return
        }
        opendialog(`/VisorInformesSti.aspx?ReporteID=2200&Anio=${desde[0]}&Mes=${desde[1]}&Dia=${desde[2]}&AnioF=${anio}`)
    })
})
